...
numero_simulaciones = 3
mlp.load_state_dict(torch.load("mlp_models_dis2grad/mlp_model_todos_das.pt"))
#mlp.load_state_dict(torch.load("mlp_models_dis2grad/mlp_model_todos_das.pt",map_location=torch.device('cpu')))
modos_desp = np.loadtxt('SVD/Biaxial2/Modos_Desplazmaientos.txt')
modos_grad = np.loadtxt('SVD/Biaxial2/Modos_Gradientes.txt')
...
plt.savefig('Histograma_MSE_disp_DAS')
...
plt.savefig('Histograma_MSE_DAS')
...
plt.savefig('Errores_Relativos_DAS')
...
#mesh.plot()
mesh.save('Grafico_errores_DAS.vtk')