...
if __name__ == '__main__':
    ...
    ##################################### Seleccion Aleatoria ######################################################
    #Se utilizan 25 modos, por lo que se seleccionan aleatoriamente 
    #ManejoDatos(D[:,-1],3).Magnitud()
    # Se seleccionan los de las caras superiores
    cara = []
    for it,i in enumerate(mesh.points):
        if i[-1] != 0:
            cara.append(it)
    ...
    #Se toman las coordenadas y puntos de los puntos seleccionados
    puntos = []
    indices = []
    for i in range(13):  ### EL numero 13 depende del numero de bases con las que se trabajen 
        punto = int(random.uniform(0,len(cara)))
        indices.append(cara[punto])
        puntos.append(np.array(mesh.points[cara[punto]]))


    # Se desarrolla un Array para graficar los samples
    mesh_samples = []

    for it,i  in enumerate(mesh.points):
        if it in indices :
            mesh_samples.append(1)

        else:
            mesh_samples.append(0)

    #### Se seleccionan los puntosa que formaran el sistema
    desplazamientos_samples = []
    indices_xy = []
    for i in indices:
        desplazamientos_samples.append(D[3*i,-1])
        desplazamientos_samples.append(D[3*i+1,-1])
        indices_xy.append(3*i)
        indices_xy.append(3*i+1)

    # Se cargan los Modos de los desplazamientos conocidos
    Modos_desp = np.loadtxt('SVD/Biaxial2/Modos_Desplazmaientos.txt') 
    A = Modos_desp[indices_xy[:-1],:]
    coef = np.linalg.solve(A,np.array(desplazamientos_samples[:-1]))
